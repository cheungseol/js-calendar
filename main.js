...
var week = new Array('一','二','三','四','五','六','七','日');
//公历节日
var sFtv = new Array(
"0101 元旦",
"0214 情人节",
...
//获取下拉列表中当前年月,显示日历信息
function start() {
   var selectYear = document.getElementById("SY");
   var selectMonth = document.getElementById("SM");
   selectYear.selectedIndex=tY-1900;
   selectMonth.selectedIndex=tM;
   drawCld(tY,tM);
}
//更改年月时,显示日历信息
function changeCld(addy,addm) {
   var selectYear = document.getElementById("SY");
   var selectMonth = document.getElementById("SM");
   var y,m;
   selectYear.selectedIndex  += addy;
   selectMonth.selectedIndex += addm;
   y=selectYear.selectedIndex+1900;
   m=selectMonth.selectedIndex;
   if(m==-1){
...
function calendar(y,m) {
   ...
   this.prevSolarMonthDays = solarMonth[m-1]; //上个月一共有多少天
...
function history() {
    ...
    document.getElementById("his").href = 'http://hao.360.cn/histoday/' + tm + tD+ '.html';
}
...
function drawCld(SY,SM) {
   ...
   var prevCld = new calendar(SY,SM-1);
   var nextCld = new calendar(SY,SM+1);
   var currentTime = document.getElementById("currentTime");
   currentTime.innerHTML = SY + " 年 " + (++SM) + " 月 " ;   //当前时间
   var currentWeekDay = document.getElementById("currentWeekDay");
   currentWeekDay.innerHTML = '星期' + week[weekday];
   var currentLunarYear = document.getElementById("currentLunarYear");
   currentLunarYear.innerHTML = Animals[(SY-4)%12];   //生肖
   var animal = document.getElementById("animal");
   animal.src = "img/" + ((SY-4)%12+1) + ".png";
   for(i=0;i<42;i++) {
      sObj=eval('SD'+ i);
      lObj=eval('LD'+ i);
      sObj.className = '';
      sD = i - cld.firstWeek;     
      if(sD>-1 && sD<cld.length) { //日期1号到月末最后一天
         sObj.innerHTML = sD+1;
         if(cld[sD].isToday){ sObj.style.color = 'red'; sObj.style.fontSize = '18px'; } //今日颜色
         else{sObj.style.color = '#2A600E'; sObj.style.fontSize = '18px';}    //普通日期颜色
         if(cld[sD].lDay==1){ //显示农历月
           lObj.innerHTML = '<b>'+(cld[sD].isLeap?'闰':'') + cld[sD].lMonth + '月' + (monthDays(cld[sD].lYear,cld[sD].lMonth)==29?'小':'大')+'</b>';
         }
         else{lObj.innerHTML = cDay(cld[sD].lDay);
          lObj.style.color = '#5A5D5A'; lObj.style.fontWeight = 'bold';} //显示农历日
        var Slfw=Ssfw=null;
        ...
        sObj.innerHTML = sD+1+cld.prevSolarMonthDays;
        sObj.style.color = '#89B181';
        lObj.innerHTML = cDay(prevCld[-1-sD].lDay); 
        lObj.style.color = '#9AA29A';
      }else if( sD>=cld.length ){
        sObj.innerHTML = sD - cld.length +1;
        sObj.style.color = '#89B181';
        lObj.innerHTML = cDay(nextCld[sD - cld.length +1].lDay); 
        lObj.style.color = '#9AA29A';
      }
   }
}
console.log("北京交通大学-张雪-研二-360前端星计划\n\n\n");
console.log("个人博客： %c http://blog.csdn.net/pepping798","color:red");
